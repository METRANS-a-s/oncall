...
def init(config):
    ...
    if config['conn'].get('migrate_on_startup'):
        ...
        cursor.execute('''UPDATE `oncall`.`role` SET `display_name`='L1' WHERE `display_order`=1''')
        cursor.execute('''UPDATE `oncall`.`role` SET `display_name`='L2' WHERE `display_order`=2''')
        cursor.execute('''UPDATE `oncall`.`role` SET `display_name`='L3 + DevOps' WHERE `display_order`=3''')
        cursor.execute('''UPDATE `oncall`.`role` SET `name`='primary' WHERE `display_order`=1''')
        cursor.execute('''UPDATE `oncall`.`role` SET `name`='secondary' WHERE `display_order`=2''')
        cursor.execute('''UPDATE `oncall`.`role` SET `name`='shadow' WHERE `display_order`=3''')
